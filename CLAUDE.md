...
**The prototype is the single source of truth for the ranking/render stages — they are not reimplemented.** `backend/app/recommender.py` imports `prototype/recommend.py` (via `sys.path`) for `compact_candidate` / `rank` / the hallucination guard, building a `TasteProfile` from the user's visit history. So `prototype/recommend.py` is live production logic, not throwaway — changing those stages means editing the prototype, and both the CLI demo and the `POST /recommendations` endpoint pick it up.

**Stage 1 retrieval, however, was moved into SQL** (`recommender._sql_retrieve`) and no longer uses the prototype's `retrieve()`: price + a geo bounding box (indexed `latitude`/`longitude`) + cuisine (`categories_text`) + rating pre-rank run in the query; only the exact circular radius and open-hours refine in Python. The three columns are derived from `location`/`categories` at seed time. `_sql_retrieve` returns the same candidate *set and order* as the prototype's `retrieve()`: both pre-rank by `rating` desc, then `rating_count` desc, then `id` asc as a deterministic tiebreaker, so results are reproducible and the two paths match exactly. On Postgres this becomes a `geography`/GIST radius query + GIN cuisine index + pgvector pre-rank (TDD §4.1 / §5.2).

**The Stage-1 source is selectable** via `RECS_PROVIDER` (`seed` default | `google`), dispatched in `recommender._retrieve_candidates`. `seed` is the SQL path above; `google` calls the live `app/providers/google_places.py` Text Search provider, which returns the identical seed-dict shape and pre-rank order (a genuine drop-in). Cuisine is folded into the text query for Google (`_google_query`) since Text Search has no structured cuisine filter. The Google path **falls back to `_sql_retrieve` on any `GooglePlacesError`** (missing key / API error), so the user always gets results — the same reliability guarantee the ranker's offline fallback gives; `result.retrieval` (and the `recommendation_logs` context) records which source actually served the request.

**Dev/prod split (the NYC cutover).** `seed` is the offline dev/test fixture (the static Philadelphia Yelp data — `pytest` runs entirely on it, no key/network). `google` is the production setting for the **NYC** launch market: it write-through-caches results into `restaurants` (`app/cache.py`; `expires_at` TTL + refresh-on-read on `GET /restaurants/{id}` via Place Details), resolves `near` for NYC (`recommender._resolve_location` layers the seed index → curated `app/geo_nyc.py` → the Geocoding API), and defaults a location-less request to the NYC center (`RECS_DEFAULT_CENTER`, else the Manhattan centroid). Google being the *default* source and the caching-to-`restaurants` ToS framing are tracked in `Design Docs/nyc-google-migration-plan.md`; live calls need `GOOGLE_MAPS_API_KEY` in `backend/.env`.
...
python -m uvicorn app.main:app              # UI at http://127.0.0.1:8000/ · docs at /docs
pytest                                       # e2e tests against an isolated temp SQLite DB
```
Run the server **from `backend/`**. Invoke it as `python -m uvicorn app.main:app` — the `uvicorn` console script is **not on PATH** in this environment (deps are a `--user` install), so the bare `uvicorn app.main:app` command fails with `command not found`. On startup, watch for `Uvicorn running on http://127.0.0.1:8000`, then hit `/health` (returns `{"status":"ok"}`) or `/app/` (the dev UI). It binds `127.0.0.1:8000` by default; pass `--port N` to change. Avoid plain `--reload`: WatchFiles watches the tree including `app.db`, so every list/visit write restarts the server mid-request (looks like a crash). Use `--reload --reload-exclude "*.db*"` if you want hot-reload. The dev DB default is an absolute path to `backend/app.db`, so the seed is used regardless of CWD.
...
- **The backend extends the TDD's draft schema.** `restaurant_notes` (per-user-per-restaurant note + tags), `list_items.source`, and `visits.sentiment` are required by PRD §4.1 but absent from the TDD §5.1 tables. They live in `backend/app/models.py`; fold them back into the TDD so docs and code stay in agreement. Recording a visit (`POST /visits`) is not just a log write — it reconciles the core lists (removes from Want-to-Try, adds to Visited), and rejects a `visited_at` dated after today.
- **Notes & tags belong to the restaurant, not the list item.** They live on `restaurant_notes` (one row per user+restaurant), so an annotation follows a restaurant across every list it's in. The `/lists/{id}/items` endpoints still accept and return `note`/`tags` (a convenience write-through / hydration), but the source of truth is the shared record, also editable directly via `GET|PUT /restaurants/{id}/note`. `list_items.source` is genuinely per-save and stays on the item. Migration `a1b2c3d4e5f6` moved the data and dropped the old columns; **tests seed via `create_all`, so the schema there comes from `models.py`, not the migration** — keep the two in sync.
